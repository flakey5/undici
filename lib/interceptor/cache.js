...
module.exports = (opts = {}) => {
  ...
  if (type !== 'shared' && type !== 'private') {
    throw new TypeError(`expected opts.type to be shared, private, got ${typeof type}`)
  }

  if (statusCodes !== true && !Array.isArray(statusCodes)) {
    throw new TypeError(`expected opts.statusCodes to be true or an array, got ${typeof statusCodes}`)
  }

  if (Array.isArray(statusCodes)) {
    for (const statusCode of statusCodes) {
      if (typeof statusCode !== 'number') {
        throw new TypeError(`expected member of opts.statusCode to be a number, got ${statusCode === null ? 'null' : typeof statusCode}`)
      }

      if (statusCode < 200 || statusCode > 599) {
        throw new TypeError(`expected member of opts.statusCode to be within range 200 <= statusCode <= 599, got ${statusCode}`)
      }

      if (statusCode === 206) {
        throw new TypeError('Undici can\'t cache partial requests yet')
      }
    }
  }
